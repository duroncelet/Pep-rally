...
export default function BuildPage() {
  ...
  async function deleteListing(id: string) {
    const response = await fetch("/api/creator-apps", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ id }) });
    const data = await response.json();
    setStatus(response.ok ? "Draft deleted." : data.error ?? "The draft could not be deleted.");
    if (response.ok) await refreshApps();
  }
  ...
  return <main className="build-page">
    ...
    <section className="build-workbench">
      ...
      <aside className="live-build-preview"><small>YOUR WORKING IDEA</small><h2>{name || "Untitled planner"}</h2><p>{outcome || "The finished outcome will appear here as you describe it."}</p><div><span><b>FOR</b>{buyer || "Your future customer"}</span><span><b>ASKS</b>{questions || "The minimum useful questions"}</span><span><b>CREATES</b>{result || "A practical finished result"}</span></div><h3>What it can use</h3>{chosen.length ? <ul>{chosen.map((item) => <li key={item.id}>{item.name}<small>{item.status}</small></li>)}</ul> : <p>No extras selected yet.</p>}<em>Drafting is free. Before publication, Pep Rally shows which connections work now and which require setup.</em></aside>
    </section>
    <section className="maker-studio" id="my-rallies"><header><small>MY RALLY STUDIO</small><h2>Your work, listings, and launches.</h2><p>See what is ready, what needs work, and what has sold. Published metrics are real—even when the honest number is zero.</p></header><div className="creator-loop"><span><b>1 · Make</b>Describe the outcome or bring a working app.</span><span><b>2 · Check</b>Add a preview, permissions, support, and price.</span><span><b>3 · Publish</b>Your working listing enters the marketplace.</span><span><b>4 · Earn</b>Paid purchases unlock buyer access and appear here.</span></div>{apps.length ? <div className="studio-grid">{apps.map((app) => { const quality = [app.outcome.trim().length >= 40, app.proof.trim().length >= 20, app.sourceType === "live" && Boolean(app.sourceUrl)].filter(Boolean).length; return <article key={app.id}><div><small>{app.stage.toUpperCase()}</small><span>{app.accessModel === "free" ? "Free" : `$${(app.priceCents / 100).toFixed(0)}`}</span></div><h3>{app.name}</h3><p>{app.outcome}</p><div className="studio-metrics"><span><b>{quality}/3</b>launch checks</span><span><b>{app.analytics?.sales ?? 0}</b>sales</span><span><b>${((app.analytics?.earningsCents ?? 0) / 100).toFixed(0)}</b>earned</span></div><em>{app.sourceType === "live" ? "Live app linked" : app.sourceType === "code" ? "Project uploaded" : "Building inside Pep Rally"}</em><div className="studio-actions">{app.stage === "published" ? <button onClick={() => changeListing(app.id, "unpublish")}>Remove from marketplace</button> : app.sourceType === "live" && app.sourceUrl ? <button className="primary" onClick={() => changeListing(app.id, "publish")}>Publish to marketplace →</button> : <b>Next: prepare the hosted working version</b>}{app.stage !== "published" && <button className="danger-link" onClick={() => deleteListing(app.id)}>Delete draft</button>}</div></article>; })}</div> : <div className="studio-empty"><b>Your first Rally will appear here.</b><p>Start with an idea or bring something you already made.</p></div>}{status && <p className="studio-status">{status}</p>}</section>

    <section className="connection-model"><header><small>BUILT FOR LAUNCH</small><h2>Everything around the mini-app, together.</h2><p>Pep Rally packages the pieces that turn a useful personal tool into something other people can find, trust, and use.</p></header><div><article><b>1</b><h3>Working now</h3><p>Listings, previews, saved access, file uploads, weather, verified reviews, and Stripe test checkout.</p></article><article><b>2</b><h3>Creator launch check</h3><p>Pep Rally reviews the outcome, permissions, source, price, support, and working URL before publication.</p></article><article><b>3</b><h3>Before real payouts</h3><p>Creator identity, Stripe Connect, tax handling, refunds, and production messaging must be activated.</p></article></div></section>
  </main>;
}
