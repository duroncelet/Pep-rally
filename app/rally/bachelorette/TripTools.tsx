...
type Props = { city: string; start: string; end: string; count: number; onCity: (city: string) => void; onDates: (start: string, end: string) => void; onAdd: (title: string, url: string, category: string) => void; onPack: (items: string[]) => void; onPlaces: () => void; onPacking: () => void; onEditDetails: () => void };
...
export default function TripTools({ city, start, end, count, onCity, onDates, onAdd, onPack, onPlaces, onPacking, onEditDetails }: Props) {
  ...
  return <section className="rally-content trip-tools">
    ...
    <div className="trip-notice"><b>{city}</b> · {start} to {end} <button onClick={onEditDetails}>Edit trip details</button></div>
    {notice && <p role="status" className="trip-notice">{notice} <button onClick={onPlaces}>Go to saved places ↓</button></p>}
    {destination ? <>
      ...
      <div className="trip-grid"><article><h3>One main activity. One backup.</h3><p><b>Start here:</b> {destination.activity}</p><p><b>If plans change:</b> {destination.backup}</p><button onClick={() => add(destination.activity, `https://www.google.com/maps/search/${encodeURIComponent(`${destination.activity} ${city}`)}`, "Activity")}>Save activity idea</button><button onClick={onPlaces}>View saved places</button></article>
      <article><h3>Outfit ideas, not requirements.</h3><p>{destination.outfit}</p><p>These are optional prompts. Add only what your group actually agrees to bring.</p><button onClick={() => { onPack([destination.outfit, "Weather-ready layer", "Venue dress-code check", "Comfortable backup shoes"]); setNotice("Optional outfit prompts added to your checklist."); }}>Add optional outfit prompts</button><button onClick={onPacking}>Open packing list</button></article></div>
