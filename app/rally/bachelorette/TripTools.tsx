...
export default function TripTools({ city, start, end, count, onCity, onDates, onAdd, onPack }: Props) {
  ...
  return <section className="rally-content trip-tools">
    ...
    <div className="destination-pills">{destinations.map(d => <button key={d.id} aria-pressed={destination?.id === d.id} onClick={() => { onCity(`${d.name}, ${d.state}`); setWeather(null); setConcerts([]); setEventMessage(""); }}>{d.name}</button>)}</div>
