...
export default function BacheloretteRally() {
  ...
  const save=async()=>{const response=await fetch("/api/party-hub",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({version:2,guests,tasks,expenses,events,packing,perPerson,paymentLink,feedback})});if(response.ok){setSaved(true);setTimeout(()=>setSaved(false),1600);}};
  const submitFeedback=async()=>{await save();setFeedbackSaved(true);};
  const createStripeLink=async()=>{setPaymentStatus("Connecting to Stripe…");const response=await fetch("/api/payment-link",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({amount:perPerson,title:"Palm Springs group share"})});const data=await response.json();if(response.status===401&&data.signIn){window.location.href=data.signIn;return;}if(response.ok){setPaymentLink(data.url);setPaymentStatus("Live Stripe payment link created ✓");}else setPaymentStatus(data.error||"Could not create a link");};
  ...
  return <main className="rally-room">
    ...
    {tab==="money"&&<div className="stripe-connect-bar"><div><small>REAL PAYMENT COLLECTION</small><b>Use any payment URL—or connect Stripe.</b><span>Pep Rally tracks who owes what. Your chosen provider securely moves the money.</span></div><button onClick={createStripeLink}>Create live Stripe link</button>{paymentStatus&&<strong>{paymentStatus}</strong>}</div>}
