...
export default function BacheloretteRally() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (plan?.version === 6 || plan?.version === 5 || plan?.version === 4 || plan?.version === 3) {
        setContributionTarget(plan.contributionTarget ?? 0); setBrideName(plan.brideName); setCity(plan.city); setStartDate(plan.startDate); setEndDate(plan.endDate); setVibe(plan.vibe); setBrideTraits(plan.brideTraits); setMustAvoid(plan.mustAvoid);
  ...
  const { groupBudget, participantCount, collected, outstanding } = tripTotals(guests, perPerson, contributionTarget);
  ...
  const updateText = configured ? `${partyName} update: ${city}, ${startDate || "dates coming soon"}. The estimated spending limit is ${perPerson} per person; the agreed contribution request is ${contributionTarget}. ${outstanding ? `$${outstanding.toLocaleString()} is still outstanding across the group.` : "No requested contributions remain. Paid bills are tracked separately."}` : "Set up the bride, dates, and guest list to create a group update.";
  ...
  const lodgingCeiling = Math.min(groupBudget, stayBudgetHigh, ...(budgetResponses.length ? [Math.min(...budgetResponses) * budgetResponses.length] : []));
  ...
  function snapshot() { return { version: 6, contributionTarget, packedItems, bills, brideName, city, startDate, endDate, vibe, brideTraits, mustAvoid, guests, tasks, expenses, events, venues, messages, packing, perPerson, paymentProvider, paymentHandle, paymentLink, stayBudgetHigh, stayNights, stayBedrooms, stayMustHaves, budgetResponses, stayOptions }; }
  async function save() {
    setSaveError("");
    try {
  ...
  function applyItinerary(artifact: ItineraryArtifact, answers: Record<string, string>) {
    ...
    setCity(answers.city); setVibe(answers.vibe); setPerPerson(artifact.totalPerPerson); setStayBudgetHigh(Math.round(artifact.totalPerPerson * Number(answers.headcount) * .4));
  ...
  function makeProviderLink() {
    ...
    const amount = Math.max(0, contributionTarget);
    if (!amount) { setPaymentStatus("Set an agreed contribution amount before making a request link."); return; }
  ...
  const nav = ["plan", "guests", "money", "packing"] as const;
  const activeNav = ["setup","stay","destination"].includes(tab) ? "plan" : tab === "chat" ? "guests" : tab;
  const labels: Record<string, string> = { plan: "Your weekend", setup: "Trip details", guests: "People", money: "Budget & bills", stay: "Stay", destination: "Explore & book", packing: "Packing", chat: "Notes & sharing" };
  ...
  return <main className="rally-room bach-room">
    ...
    {workspaceReady && <>
    <nav className="rally-nav">{nav.map((item) => <button key={item} className={activeNav === item ? "active" : ""} onClick={() => { setTab(item); setNotice(""); }}>{labels[item]}</button>)}</nav>
    {activeNav === "plan" && <div className="rally-subnav">{(["plan","stay","destination","setup"] as const).map(key => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{({plan:"Itinerary",stay:"Stays",destination:"Places & activities",setup:"Edit trip details"})[key]}</button>)}</div>}
    {activeNav === "guests" && <div className="rally-subnav"><button onClick={() => setTab("guests")}>Guest list</button><button onClick={() => setTab("chat")}>Organizer notes</button></div>}
    {notice && <p role="status" className="trip-notice">{notice}</p>}
    ...
    {tab === "money" && <section className="rally-content"><div className="workspace-intro"><div><small>MONEY</small><h2>Plan spending. Track actual payments.</h2><p>Pep Rally tracks balances; the selected payment provider moves the funds securely.</p></div><label>Estimated budget per person<input type="number" min="0" value={perPerson} onChange={(e) => setPerPerson(Number(e.target.value))}/></label></div><div className="money-strip"><span>Estimated group budget <b>${groupBudget.toLocaleString()}</b></span><span>Contributions recorded <b>${collected.toLocaleString()}</b></span><span>Requested contributions remaining <b>${outstanding.toLocaleString()}</b></span><span>Planned expenses <b>${expenseTotal.toLocaleString()}</b></span></div><h3>Group contributions</h3><p>Optional: agree a contribution with the group before requesting it. This is separate from your spending estimate and the paid-bill splitter below.</p><label>Agreed contribution per person $<input type="number" min="0" value={contributionTarget} onChange={e => setContributionTarget(Math.max(0, Number(e.target.value)))}/></label><div className="payment-connect"><div><label>Provider<select value={paymentProvider} onChange={(e) => setPaymentProvider(e.target.value)}><option>PayPal.Me</option><option>Cash App</option><option>Venmo</option><option>Other link</option></select></label><label>{paymentProvider === "Other link" || paymentProvider === "Stripe" ? "Payment link" : "Organizer handle"}<input value={paymentProvider === "Other link" || paymentProvider === "Stripe" ? paymentLink : paymentHandle} onChange={(e) => paymentProvider === "Other link" || paymentProvider === "Stripe" ? setPaymentLink(e.target.value) : setPaymentHandle(e.target.value)} placeholder={paymentProvider === "PayPal.Me" ? "paypal.me name" : paymentProvider === "Cash App" ? "$cashtag" : paymentProvider === "Venmo" ? "@username" : "https://…"}/></label></div><div>{paymentProvider === "Stripe" ? <span>Use a payment link owned by the actual recipient.</span> : paymentProvider !== "Other link" && <button onClick={makeProviderLink}>Make request link</button>}{paymentLink && <a href={paymentLink} target="_blank" rel="noreferrer">Open payment request ↗</a>}</div><small>{paymentStatus || "Provider accounts and their terms apply. Mark payments received in the ledger below."}</small></div><div className="balance-list">{guests.map((guest, index) => guest.rsvp !== "No" && <article key={guest.id}><div><b>{guest.name}</b><small>${Math.max(0, contributionTarget - guest.paid).toLocaleString()} of requested contribution remaining</small></div><label>Paid $<input type="number" min="0" value={guest.paid} onChange={(e) => setGuests(guests.map((item, i) => i === index ? { ...item, paid: Number(e.target.value) } : item))}/></label><div className="balance-meter"><i style={{ width: `${contributionTarget ? Math.min(100, guest.paid / contributionTarget * 100) : 0}%` }}/></div></article>)}</div><div className="rally-title compact"><div><small>EXPENSE PLAN</small><h3>What the group budget covers</h3></div><button onClick={() => setExpenses([...expenses, { id: crypto.randomUUID(), label: "New expense", amount: 0, paidBy: owners[0].name }])}>+ Add expense</button></div><div className="expense-list">{expenses.map((expense, index) => <div key={expense.id}><input value={expense.label} onChange={(e) => setExpenses(expenses.map((item, i) => i === index ? { ...item, label: e.target.value } : item))}/><label>$ <input type="number" value={expense.amount} onChange={(e) => setExpenses(expenses.map((item, i) => i === index ? { ...item, amount: Number(e.target.value) } : item))}/></label><select value={expense.paidBy} onChange={(e) => setExpenses(expenses.map((item, i) => i === index ? { ...item, paidBy: e.target.value } : item))}>{owners.map((guest) => <option key={guest.id}>{guest.name}</option>)}</select><button onClick={() => setExpenses(expenses.filter((item) => item.id !== expense.id))}>×</button></div>)}</div><BillSplitter guests={guests} bills={bills} onChange={setBills}/></section>}

    {tab === "plan" && <section className="rally-content"><div className="workspace-intro"><div><small>ITINERARY</small><h2>{events.length ? `${vibe}, with breathing room.` : "Generate a plan from the brief."}</h2><p>Your draft is ready. Rename guests, choose actual places and adjust these suggestions to fit the group.</p></div><div className="rally-actions"><button onClick={() => setEvents([...events, { id: crypto.randomUUID(), day: 1, time: "12:00 PM", title: "", owner: owners[0].name, status: "Idea" }])}>+ Add your own activity</button><button onClick={() => setTab("destination")}>Find places →</button><button className="primary" onClick={downloadOutcome}>Download weekend plan</button></div></div><div className="event-list rich-events">{events.map((event, index) => <details className="weekend-card" key={event.id}><summary><span>{startDate ? new Date(Date.parse(startDate + "T12:00:00") + (event.day - 1) * 86400000).toLocaleDateString(undefined, { weekday:"long", month:"short", day:"numeric" }) : `Day ${event.day}`} · {event.time}</span><strong>{event.title || "New activity"}</strong><small>{event.status} · Edit</small></summary><article><label>Day<input type="number" min="1" max="32" value={event.day} onChange={(e) => setEvents(events.map((item, i) => i === index ? { ...item, day: Number(e.target.value) } : item))}/></label><label>Time<input value={event.time} onChange={(e) => setEvents(events.map((item, i) => i === index ? { ...item, time: e.target.value } : item))}/></label><input aria-label="Activity name" placeholder="Your activity" className="event-name" value={event.title} onChange={(e) => setEvents(events.map((item, i) => i === index ? { ...item, title: e.target.value } : item))}/><select value={event.owner} onChange={(e) => setEvents(events.map((item, i) => i === index ? { ...item, owner: e.target.value } : item))}>{owners.map((guest) => <option key={guest.id}>{guest.name}</option>)}</select><select value={event.status} onChange={(e) => setEvents(events.map((item, i) => i === index ? { ...item, status: e.target.value as Event["status"] } : item))}><option>Idea</option><option>Shortlisted</option><option>Booked</option></select>{event.bookingUrl && <a href={event.bookingUrl} target="_blank" rel="noreferrer">Venue ↗</a>}<button aria-label={`Remove ${event.title || "activity"}`} onClick={() => setEvents(events.filter((item) => item.id !== event.id))}>×</button></article></details>)}</div></section>}
    ...
    {tab === "chat" && <section className="rally-content"><button className="primary" onClick={downloadOutcome}>Download weekend plan</button></section>}
    {tab === "plan" && <details className="rally-content"><summary>Customize in another AI tool</summary><p>Download prompts and your trip details to use in another AI tool.</p><button onClick={downloadCustomization}>Customize this Rally .md</button></details>}
    </>}
  </main>;
}
