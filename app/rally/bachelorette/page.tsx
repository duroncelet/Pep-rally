...
function PackingSection({ packing, onChange, onDownload }: { packing: string[]; onChange: (items: string[]) => void; onDownload: () => void }) {
  const [custom, setCustom] = useState("");
  const [notice, setNotice] = useState("");
  const add = (item: string) => { const value = item.trim(); if (!value || packing.includes(value)) return; onChange([...packing, value]); setNotice(`${value} added to your group list.`); };
  async function share() {
    const text = `Bachelorette packing list\n${packing.map(item => `□ ${item}`).join("\n")}`;
    try { if (navigator.share) await navigator.share({ title: "Bachelorette packing list", text }); else { await navigator.clipboard.writeText(text); setNotice("Packing list copied. Paste it into the group chat."); } } catch { setNotice("Copy cancelled. Use Download plan .md to share a portable file."); }
  }
  return <section className="rally-content"><div className="rally-title"><div><small>PACKING + SAFETY</small><h2>Build the list your group will actually use.</h2><p>Nothing is pre-populated. These are optional suggestions based on weather, the itinerary and common bachelorette planning guidance.</p></div><div className="rally-actions"><button onClick={share}>Share list</button><button onClick={onDownload}>Download plan .md</button><button onClick={() => window.print()}>Print / save as PDF</button></div></div>{notice && <p role="status" className="trip-notice">{notice}</p>}<article className="packing-suggestions"><small>OPTIONAL SUGGESTIONS</small><div>{packingSuggestions.filter(item => !packing.includes(item)).map(item => <button key={item} onClick={() => add(item)}>+ {item}</button>)}</div></article><form className="add-packing" onSubmit={e => { e.preventDefault(); add(custom); setCustom(""); }}><label>Add your own item<input value={custom} onChange={e => setCustom(e.target.value)} placeholder="e.g. room key envelope, allergy-safe snacks" /></label><button type="submit">Add item</button></form><div className="packing-list">{packing.length ? packing.map((item, index) => <label key={`${item}-${index}`}><input type="checkbox"/><input value={item} onChange={(e) => onChange(packing.map((value, i) => i === index ? e.target.value : value))}/><button type="button" onClick={() => onChange(packing.filter((_, i) => i !== index))}>×</button></label>) : <p className="empty-state">Your real list starts here. Add only what someone has agreed to bring.</p>}</div><div className="safety-card"><b>One boring but essential check</b><p>Confirm the lodging address, emergency contact, transportation home, medication storage, accessibility needs and who is responsible for shared items.</p></div></section>;
}
...
export default function BacheloretteRally() {
  const [tab, setTab] = useState<"overview" | "destination" | "setup" | "guests" | "stay" | "chat" | "money" | "plan" | "places" | "decor" | "tasks" | "packing">("overview");
  ...
  async function save() {
    setSaveError("");
    try {
    const response = await fetch("/api/party-hub", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ version: 5, bills, brideName, city, startDate, endDate, vibe, brideTraits, mustAvoid, guests, tasks, expenses, events, venues, messages, packing, perPerson, paymentProvider, paymentHandle, paymentLink, stayBudgetHigh, stayNights, stayBedrooms, stayMustHaves, budgetResponses, stayOptions }) });
  ...
  function applyItinerary(artifact: ItineraryArtifact, answers: Record<string, string>) {
    ...
    setEvents(artifact.days.flatMap((day, dayIndex) => day.items.map(item => ({ id: crypto.randomUUID(), day: dayIndex + 1, time: item.time, title: item.title, owner: guests[0]?.name || "Organizer", status: "Idea" as const }))));
  ...
  return <main className="rally-room bach-room">
    ...
    {tab === "packing" && <PackingSection packing={packing} onChange={setPacking} onDownload={downloadOutcome}/>}
  </main>;
}
