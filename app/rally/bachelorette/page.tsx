...
export default function BacheloretteRally() {
  ...
  return <main className="rally-room bach-room">
    <header className="rally-header photo-rally-header bach-header"><a href="/" className="brand"><span className="brand-mark">P</span>Pep Rally</a><div className="rally-header-copy"><small>BACHELORETTE BLUEPRINT · ORGANIZER WORKSPACE</small><h1>{partyName}</h1>{workspaceReady && <p>{city} · {startDate}–{endDate}</p>}</div><figure className="rally-cover"><img src="/rallies/bachelorette-pool.jpg" alt="Friends relaxing together beside a sunny pool"/><figcaption><b>Free Rally</b><span>Plan here; review before you send or book</span></figcaption></figure><div className="rally-header-actions" hidden={!workspaceReady}>{authState === "guest" ? <><button onClick={downloadCustomization}>Customize this Rally .md</button><button onClick={downloadOutcome}>Download my plan .md</button><button className="primary" onClick={() => keepThisRally()}>Sign in to keep it</button></> : <><button onClick={downloadCustomization}>Customize this Rally .md</button><button onClick={downloadOutcome}>Download my plan .md</button><button className="primary" onClick={save}>{saved ? "Saved ✓" : "Save Rally"}</button></>}</div></header>
