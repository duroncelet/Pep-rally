...
export async function POST(request: Request) {
  const user = await getChatGPTUser();
  if (!user) return Response.json({ error: "Sign in required", signIn: chatGPTSignInPath("/") }, { status: 401 });
  const body = await request.json() as { name?:string; problem?:string; outcome?:string; proof?:string; accessModel?:string; price?:number; sourceType?:string; sourceUrl?:string; sourceFileKey?:string; builderSpec?:string; parentSlug?:string; parentTitle?:string };
