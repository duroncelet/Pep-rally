...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as { creatorAppId?: string };
  const db = getDb();
  ...
  const amountCents = creatorApp?.priceCents ?? 100;
  const title = creatorApp?.name ?? "Bachelorette Blueprint · payment proof";
  const toolSlug = creatorApp ? `creator-app:${creatorApp.id}` : "bachelorette-blueprint";
  ...
  try {
    const session = await createCheckoutSession(secret, {
      ...
      title,
      description: creatorApp?.outcome ?? "A Stripe test-mode checkout that unlocks a saved executable workspace. No real money moves.",
      amountCents,
      toolSlug,
