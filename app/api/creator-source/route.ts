...
export async function POST(request: Request) {
  ...
  if (!env.ASSETS) return Response.json({ error: "Upload storage is unavailable" }, { status: 503 });
  const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "-").slice(-100);
  const key = `creator-sources/${user.userId}/${crypto.randomUUID()}-${safeName}`;
  await env.ASSETS.put(key, file.stream(), { httpMetadata: { contentType: file.type || "application/octet-stream" }, customMetadata: { owner: user.userId, originalName: file.name } });
