...
export default function Home() {
  ...
  return <main className="consumer-home">
    ...
    <section className="community-marketplace" id="marketplace"><header><div><small>THE MARKETPLACE</small><h2>Small apps. Real outcomes.</h2><p>Made by people who found a better way to handle something—and decided to share it.</p></div><a href="/build">Sell your own Rally →</a></header>{listings.length ? <div className="community-grid">{listings.map((listing, index) => <article key={listing.id} className={`community-card tone-${index % 4}`}><div className="community-card-top"><span>Made by {listing.creatorName}</span><b>{listing.accessModel === "free" ? "Free" : `$${(listing.priceCents / 100).toFixed(0)}`}</b></div><div className="community-icon">{listing.name.slice(0, 1).toUpperCase()}</div><h3>{listing.name}</h3><p>{listing.outcome}</p><small>{listing.proof}</small>{listing.accessModel === "free" && listing.sourceUrl ? <a className="primary center-link" href={listing.sourceUrl} target="_blank" rel="noreferrer">Open free Rally →</a> : <button className="primary" onClick={() => buyListing(listing.id)}>Get this Rally · ${(listing.priceCents / 100).toFixed(0)}</button>}</article>)}</div> : <div className="community-grid starter-grid"><article className="community-card tone-1"><div className="community-card-top"><span>Pep Rally Original</span><b>Free</b></div><div className="community-icon">B</div><h3>Bachelorette Blueprint</h3><p>Plan the people, money, stay, itinerary, reservations, and group updates in one working app.</p><small>Free launch collection · full executable workspace</small><a className="primary center-link" href="/rally/bachelorette">Open free Rally →</a></article><article className="community-card tone-2"><div className="community-card-top"><span>Pep Rally Original</span><b>Free</b></div><div className="community-icon">G</div><h3>Little Garden Planner</h3><p>Turn your real space, sunlight, setup, location, and favorite crops into a living garden plan.</p><small>Free launch collection · live weather included</small><a className="primary center-link" href="/rally/garden">Open free Rally →</a></article><article className="community-card join-card"><div className="community-card-top"><span>YOUR SHELF SPACE</span><b>Open</b></div><div className="community-icon">+</div><h3>Sell something you made</h3><p>Link a live mini-app or upload your project, choose free or paid access, and publish your listing.</p><small>Built for useful, focused apps that solve everyday problems.</small><a className="primary center-link" href="/build">Make a Rally →</a></article></div>}{marketStatus && <p className="market-status">{marketStatus}</p>}</section>
